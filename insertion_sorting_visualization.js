...
var numberOfItems = document.getElementById("number_of_items");
...
function insertionSort() {
  ...
  (function () {
    ...
    if (i < divs.length) {
      ...
      for (
        var j = i;
        j > 0 && Number(divs[j - 1].getAttribute("data-val")) > currentDiv;
        j--
      ) {
      ...
      setColor(divs, "darkgreen");
    }
  })();
}

function setColor(arr, color) {
  for (let i = 0; i < arr.length; i++) arr[i].style.background = color;
}
